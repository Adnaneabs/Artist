...
class Marketplace(sp.Contract):
    ...
    def collect(self, params):
        ...
        #Sending royalties to creator
        sp.send(self.data.data[params.token_id].author,  sp.split_tokens(sp.split_tokens(sp.amount, 97, 100), 10, 100))

        #Sending management fees
        sp.send(self.data.admin, sp.split_tokens(sp.amount, 3, 100))

        #Sending value to holder
        sp.send(self.data.data[params.token_id].holder, sp.split_tokens(sp.amount, 97, 100)-sp.split_tokens(sp.split_tokens(sp.amount, 97, 100), 10, 100))
   ...
   @sp.entry_point
   def transfer(self, fa2, from_, to_, token_id, amount):
        c = sp.contract(sp.TList(sp.TRecord(from_=sp.TAddress, txs=sp.TList(sp.TRecord(amount=sp.TNat, to_=sp.TAddress, token_id=sp.TNat).layout(("to_", ("token_id", "amount")))))), fa2, entry_point='transfer').open_some()
        sp.transfer(sp.list([sp.record(from_=from_, txs=sp.list([sp.record(amount=amount, to_=to_, token_id=token_id)]))]), sp.mutez(0), c)
